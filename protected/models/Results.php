...
class Results extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('ResultName', 'required'),
			array('EmplLock, EmplChange, EmplDel', 'numerical', 'integerOnly'=>true),
			array('ResultName', 'length', 'max'=>100),
			array('Lock, DateLock, DateChange, DelDate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('Result_Id, ResultName, Lock, EmplLock, DateLock, EmplChange, DateChange, EmplDel, DelDate', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('ResultName',$this->ResultName,true);
	 ...
	 */
	 public function deleteCount($id, $empl_id) {
		...
		$Command = Yii::app()->db->createCommand(''
                . "UPDATE Results SET EmplDel = {$empl_id}, DelDate = '".date('m.d.y H:i:s')."' WHERE Result_Id = {$id}
                ");
	...
	public static function all()
        {
            return CHtml::listData(self::model()->findAll(), 'Result_Id', 'ResultName');
        }
        
        public function getData() {
            $q = new SQLQuery();
            $q->setSelect("Select Result_Id, ResultName");
            $q->setFrom("\nFrom Results");
