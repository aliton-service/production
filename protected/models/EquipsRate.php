...
/**
 * This is the model class for table "EquipsRate".
 *
 * The followings are the available columns in table 'EquipsRate':
 ...
 */
class EquipsRate extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('eqip_id, EmplLock, EmplCreate, EmplChange, EmplDel', 'numerical', 'integerOnly'=>true),
			array('price', 'length', 'max'=>19),
			array('date_start, date_end, Lock, DateLock, DateCreate, DateChange, DelDate', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('eqrt_id, eqip_id, price, date_start, date_end, Lock, EmplLock, DateLock, EmplCreate, DateCreate, EmplChange, DateChange, EmplDel, DelDate', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return EquipsRate the static model class
	 */
	 public function deleteCount($id, $empl_id) {
	 
		$Command = Yii::app()->db->createCommand(''
                . "UPDATE EquipsRate SET EmplDel = {$empl_id}, DelDate = '".date('m.d.y H:i:s')."' WHERE eqrt_id = {$id}
                ");
