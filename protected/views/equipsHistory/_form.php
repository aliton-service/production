<?php
/* @var $this EquipsHistoryController */
/* @var $model EquipsHistory */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'equips-history-form',
	'htmlOptions'=>array(
		'class'=>'form-inline'
		),
	// Please note: When you enable ajax validation, make sure the corresponding
	// controller action is handling ajax validation correctly.
	// There is a call to performAjaxValidation() commented in generated controller code.
	// See class documentation of CActiveForm for details on this.
	'enableAjaxValidation'=>true,
)); ?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	<div class="row">
		<?php echo $form->labelEx($model,'equip_id'); ?>
		<?php echo $form->textField($model,'equip_id'); ?>
		<?php echo $form->error($model,'equip_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'achs_id'); ?>
		<?php echo $form->textField($model,'achs_id'); ?>
		<?php echo $form->error($model,'achs_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'ac_date'); ?>
		<?php echo $form->textField($model,'ac_date'); ?>
		<?php echo $form->error($model,'ac_date'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'objc_id'); ?>
		<?php echo $form->textField($model,'objc_id'); ?>
		<?php echo $form->error($model,'objc_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'docm_id'); ?>
		<?php echo $form->textField($model,'docm_id'); ?>
		<?php echo $form->error($model,'docm_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'dctp_id'); ?>
		<?php echo $form->textField($model,'dctp_id'); ?>
		<?php echo $form->error($model,'dctp_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'quant'); ?>
		<?php echo $form->textField($model,'quant'); ?>
		<?php echo $form->error($model,'quant'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'used'); ?>
		<?php echo $form->checkBox($model,'used'); ?>
		<?php echo $form->error($model,'used'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'mstr_id'); ?>
		<?php echo $form->textField($model,'mstr_id'); ?>
		<?php echo $form->error($model,'mstr_id'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'direct'); ?>
		<?php echo $form->textField($model,'direct'); ?>
		<?php echo $form->error($model,'direct'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'Lock'); ?>
		<?php echo $form->checkBox($model,'Lock'); ?>
		<?php echo $form->error($model,'Lock'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'EmplLock'); ?>
		<?php echo $form->textField($model,'EmplLock'); ?>
		<?php echo $form->error($model,'EmplLock'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'DateLock'); ?>
		<?php echo $form->textField($model,'DateLock'); ?>
		<?php echo $form->error($model,'DateLock'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'EmplCreate'); ?>
		<?php echo $form->textField($model,'EmplCreate'); ?>
		<?php echo $form->error($model,'EmplCreate'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'DateCreate'); ?>
		<?php echo $form->textField($model,'DateCreate'); ?>
		<?php echo $form->error($model,'DateCreate'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'EmplChange'); ?>
		<?php echo $form->textField($model,'EmplChange'); ?>
		<?php echo $form->error($model,'EmplChange'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'DateChange'); ?>
		<?php echo $form->textField($model,'DateChange'); ?>
		<?php echo $form->error($model,'DateChange'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'EmplDel'); ?>
		<?php echo $form->textField($model,'EmplDel'); ?>
		<?php echo $form->error($model,'EmplDel'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'DelDate'); ?>
		<?php echo $form->textField($model,'DelDate'); ?>
		<?php echo $form->error($model,'DelDate'); ?>
	</div>

	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Создать' : 'Сохранить', array('class'=>'btn btn-primary')); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->